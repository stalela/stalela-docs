              ...
              schema:
                ...
                properties:
                  items:
                    ...
                    items:
                      type: object
                      required: [transferId, state, intent, rail, amount, createdAt, updatedAt, version]
                      properties:
                        ...
                        fxStrategy: { type: string }
                        externalRef: { type: string }
                        endUserRef: { type: string }
                        ...
                        latestEventAt: { type: string, format: date-time }
...
create table transfer_summaries (
  ...
  latestEventType text null,
  latestEventAt timestamptz null,
  primary key (tenantId, transferId)
);

create index ix_summary_tenant_created on transfer_summaries(tenantId, createdAt desc, transferId);
create index ix_summary_tenant_state_created on transfer_summaries(tenantId, state, createdAt desc, transferId);
create index ix_summary_tenant_external on transfer_summaries(tenantId, externalRef) where externalRef is not null;
```
