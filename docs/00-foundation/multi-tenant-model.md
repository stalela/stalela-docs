...
```mermaid
flowchart TD
    T["Stalela Account (tenantId)"] --> M["Merchant (merchant_nif)"]
...
|---|---|---|
| `tenantId` | `merchant_nif` | 1:1 binding created during merchant onboarding |
...
1. **Account creation** → a `tenantId` is minted in the Payments Nucleus and a `merchant_nif` is registered in the Fiscal Platform.
...
3. **API keys** → each API key is scoped to both a `tenantId` (Payments) and a `merchant_nif` + `outlet_id` (Fiscal). A single Bearer token authenticates to both pillars.
...
- **Fiscal Platform**: All fiscal numbers, ledger entries, reports, and DGI sync bundles are scoped to `merchant_nif` + `outlet_id`. The Monotonic Counter Manager enforces per-outlet serialization.
